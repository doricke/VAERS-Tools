...
def report_by_dose( data )
  ...
  # for yr in 0..120 do
  sym_order.each do |symptom, all|
    for year in (0..120).step(10) do
      print "#{symptom}\t\"#{year}-#{year+9}\""
      vax_names.each do |vax_name, count|
        if vax_count[vax_name] >= 100
          g_count = 0
          g_total = 0
          for i in 0..9 do
            ...
            g_count += count if ! count.nil?
            total = vax_tally[vax_name][ yr ] if ! vax_tally[vax_name].nil?
            g_total += total if ! total.nil?
          end  # for
          freq = 0
          freq = (g_count * 100000)/g_total if ! g_total.nil? && g_total > 0
          print "\t#{g_count}|#{g_total}|#{freq.to_i}\t#{freq.to_i}"
        end  # if
      end  # do
      print "\n"
    end  # do
