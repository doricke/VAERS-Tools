    ...
    data.keys.each do |id|
      if ! data[id].nil? && ! data[id][:symptoms].nil? && ! data[id][ :vax ].nil? && data[id][:symptoms][symptom]
        data[id][ :vax ].each do |vax_record|
          ...
          tally[ vax_name ][ vax_year ] = 0 if tally[ vax_name ][ vax_year ].nil?
          tally[ vax_name ][ vax_year ] += 1
...
def shots_report( data, select )
  ...
  # Tally up the number of vaccine shots by vaccinee.
  shots_tally = {}
  select.keys.each do |symptom|
    data.keys.each do |id|
      if ! data[id].nil? && ! data[id][:symptoms].nil? && ! data[id][ :vax ].nil? && data[id][:symptoms][symptom]
        if shots_tally[id].nil?
          shots_tally[id] = 1
        else
          shots_tally[id] += 1
        end  # if
  ...
  select.keys.each do |symptom|
    data.keys.each do |id|
      if ! data[id].nil? && ! data[id][:symptoms].nil? && ! data[id][ :vax ].nil? && data[id][:symptoms][symptom]
        data[id][ :vax ].each do |vax_record|
          vax_shots = shots_tally[id]
          vax_name = vax_record[:vax_name]
          tally[ vax_name ] = {} if tally[ vax_name ].nil?
          tally[ vax_name ][ "All" ] = 0 if tally[ vax_name ][ "All" ].nil?
          tally[ vax_name ][ "All" ] += 1 
          tally[ vax_name ][ vax_shots ] = 0 if tally[ vax_name ][ vax_shots ].nil?
          tally[ vax_name ][ vax_shots ] += 1 
  ...
  vax_names.each do |vax_name, count|
    print "#{vax_name}"
    total = tally[vax_name]["All"]
    print "\t#{total}"
    for shots in 1..MAX_SHOTS do
      if tally[vax_name][shots].nil?
        print "\t0"
      else
        print "\t#{tally[vax_name][shots]}"
      end  # if
    end  # do
    print "\n"
  end  # do
